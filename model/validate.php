...
class Validate {
    ...
    public function email($name, $value, $required = true) {
        ...
        if (strlen($domain) > 255) {
            $field->setErrorMessage('Domain part too long.');
        }
    ...
    public function password($name, $password, $required = true) {
        ...
        if ($pwMatch === false) {
            ...
            $field->setErrorMessage('Must one each of upper, lower and digit.button_form');
            return;
        }
    }
    ...
    public function expDate($name, $value) {
        ...
        if ($match === false) {
            $field->setErrorMessage('Error checking field.');
            return;
        }
        if ($match != 1) {
